...
export async function startRoadGame({ container, onStatus = () => {} } = {}) {
  ...
  function updateLensFlare(dt) {
    ...
    /*
     * THE FLARE'S COLOUR IS THE SKY'S, not a constant. `getLook().sunColor` is the sun seen
     * through the current air mass and is already LINEAR (the sky builds it with
     * toLinearHex), so it goes straight across — converting it again is the 5-10x error
     * that stays self-consistent and therefore hides.
     *
     * It is a shared scratch Color inside the sky, so the flare copies it rather than
     * holding the reference.
     */
    const look = gameSky?.getLook?.();
    if (look?.sunColor) app.lensFlare?.setSourceColor?.(look.sunColor);
