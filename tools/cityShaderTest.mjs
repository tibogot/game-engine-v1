...
//
// The builder itself lives in ./wgslBuilderStub.mjs — it registers the three
// hook, so it MUST be imported before anything that pulls in three.
...
//
// punched / curtain / ribbon used to be a texture read, so all three wall
// systems compiled into one shader and every tower paid for the two it is not.
// The type is known on the CPU, so each InstancedMesh now gets a material with
// it pinned — and the win is only real if the folded branches actually LEAVE
// the generated code, which is what this measures.
//
// It also catches the failure mode that would be invisible in the game: a pin
// that folds the wrong way makes a material that still compiles and still
// draws, just with the wrong wall system on it. A curtain-wall shader that
// still contains the stone-joint code is the signature.
{
  const split = createCityFacadeMaterial();
  check("the facade exposes a pinned material per building type", split.typeSplit === true);
  const combined = (builder && builder.fragmentShader ? builder.fragmentShader : "").split(LFCH).length;
  let total = 0;
  const names = ["punched", "curtain", "ribbon"];
  const src = [];
  for (let t = 0; t < 3; t++) {
    let b = null, err = null;
    try { b = buildWGSL(split.materialFor(t, false)); } catch (e) { err = e; }
    check(`the ${names[t]} facade compiles`, err === null, err ? err.message : "");
    if (!b) continue;
    const frag = b.fragmentShader || "";
    src[t] = frag;
    const lines = frag.split(LFCH).length;
    total += lines;
    console.log(`       ${names[t].padEnd(8)} ${(frag.length / 1024).toFixed(1)} kB · ${lines} lines  (${((lines / combined) * 100).toFixed(0)}% of combined)`);
    check(`the ${names[t]} facade is smaller than the combined one`, lines < combined,
      `${lines} vs ${combined}`);
  }
  // A PIN THAT NEVER REACHED THE GRAPH still compiles, still draws, and still
  // looks right — it is just the combined shader under three names, with three
  // times the pipelines and none of the saving. Identical output between two
  // types is the signature, and it is the only failure here that is silent in
  // the game.
  check("the punched facade differs from the curtain one",
    !!src[0] && !!src[1] && src[0] !== src[1]);
  console.log(`       combined ${combined} lines · three pinned ${total} lines total`);
  // WHICH NUMBER MATTERS DEPENDS ON WHAT IS BEING PAID.
  //   per pipeline — what a wave executes, so register pressure and occupancy.
  //   the SUM      — what the driver compiles, and the city draws all three at
  //                  once, so a first frame pays every one of them.
  // The split wins the first and loses the second, which is why it is off by
  // default in CITY_DEFAULTS. This tripwire guards the half it does win.
  check("the mean pinned facade is at least 10% smaller than the combined one",
    total / 3 < combined * 0.9, `mean ${Math.round(total / 3)} vs ${combined}`);
