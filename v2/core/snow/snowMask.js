...
/**
 * World-fixed snow accumulation mask.
 *
 * - R channel = painted accumulation weight (0..255 → 0..1 in shader).
 * - Same pattern as `RevoGrassMask`: a Uint8 DataTexture covering the
 *   full world; painted by `SnowMaskPaintSystem` via `stamp()`; sampled
 *   by `SnowSystem`'s elevation pass at world UV.
 *
 * The accumulation mask is positive ("more snow here") — opposite of
 * RevoGrass's exclusion mask. The shader multiplies the painted weight
 * by `accumStrength` and adds it to the procedural target elevation, so
 * painted areas pile up additional snow on top of the global weather drive.
 */
export class SnowMask {
  constructor(res = 512) {
    this.res = res;
    const data = new Uint8Array(res * res * 4);
    /** Default = 0 (no extra painted accumulation); global weather still drives the base. */
