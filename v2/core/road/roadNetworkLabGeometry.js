/**
 * Smart Road network geometry (graph → road pieces). Originated in
 * `v2/profile-road-lab0.html`, hardened in `v2/smart-road-lab.html`.
 ...
 *    overlap with its closest angular neighbor — keeps sharp Y/trident junctions
 *    from folding through themselves.
 *  - corner reach scales with the clips (a fixed reach rejected legitimate corners
 *    once clips grew, cutting junctions below their node).
 *  - ROUNDABOUT nodes (`roundabout` flag): circulating ring + width-preserving
 *    connector flares per approach.
 *
 * Returns { ok, pieces, markings, nodes }. Marking types: center / centerLeft /
 * centerRight / divider / edge. New optional inputs are backward-compatible:
 * edge.bend (0), node.roundabout (false), params.roundaboutRadius, params.edgeLineInset.
 */
