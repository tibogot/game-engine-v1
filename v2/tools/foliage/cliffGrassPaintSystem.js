...
export class CliffGrassPaintSystem {
  ...
  beginStroke(hitPoint, event = {}) {
    ...
    this.lastStrokePoint = null;
    this.beforeSnapshot = this.grassManager.getCliffDensitySnapshot();
  ...
  applyAt(hitPoint, event = {}) {
    ...
    this.grassManager.stampCliffDensity({
      cx: hitPoint.x,
      cz: hitPoint.z,
      radius: brush.radius,
  ...
  endStroke() {
    ...
    const afterSnapshot = this.grassManager.getCliffDensitySnapshot();
    let changed = false;
  ...
  undo() {
    ...
    if (!cmd) return;
    this.grassManager.restoreCliffDensitySnapshot(cmd.before);
  ...
  redo() {
    ...
    if (!cmd) return;
    this.grassManager.restoreCliffDensitySnapshot(cmd.after);
