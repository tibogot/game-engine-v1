...
export class RiverCarvingSystem {
  /**
   * @param {object} opts
   * @param {THREE.Scene}  opts.scene
   * @param {object}       opts.toolState
   * @param {function}     opts.getWorldHeight  (x,z)=>y
   * @param {object}       opts.terrainStore    TerrainStore instance
   * @param {function}     opts.markTerrainDirty (dirtyChunks)=>void
   */
  constructor({ scene, toolState, getWorldHeight, terrainStore, markTerrainDirty }) {
    this.scene = scene;
    this.toolState = toolState;
    this.getWorldHeight = getWorldHeight;
    this.terrainStore = terrainStore;
    this.markTerrainDirty = markTerrainDirty;

    // segments: { points: Vector3[], mesh: Mesh|null }
    this.segments = [];
    this.selectedIdx = -1;
    this.dragging = false;

    // One global snapshot of terrain taken before first carve. Used to undo all carves.
    this._baseTerrainSnapshot = null;

    this.handleGroup = new THREE.Group();
    this.handleGroup.name = "RiverCarvingHandles";
  ...
  // ---------------------------------------------------------------------------
  // Shaders (identical to RiverSystem)
  // ---------------------------------------------------------------------------

  _initNodeMaterials() {
    this.uRiverTime = uniform(0.0);
    this.uRiverFlowSpeed = uniform(this.toolState.river2.flowSpeed ?? 0.15);
    this.uRiverDeepColor = uniform(new THREE.Color(this.toolState.river2.deepColor ?? "#1a4a6a"));
    this.uRiverShallowColor = uniform(new THREE.Color(this.toolState.river2.shallowColor ?? "#5dbfaa"));
    this.uRiverHighlight = uniform(new THREE.Color(this.toolState.river2.highlightColor ?? "#c8ecff"));
    this.uRiverFoamColor = uniform(new THREE.Color(this.toolState.river2.foamColor ?? "#ffffff"));
    this.uRiverFoamWidth = uniform(this.toolState.river2.foamWidth ?? 0.18);
    this.uRiverOpacity = uniform(this.toolState.river2.opacity ?? 0.88);
    ...
    this.rsU = {
      ...
      opacity: uniform(this.toolState.river2.opacity ?? 0.88),
    };
   ...
   * New chunks are snapshotted from current (pre-carve) terrain; existing
   * chunks in the base are left untouched.
   */
  _expandBaseSnapshot(pts) {
    const rp = this.toolState.river2;
    const halfW = rp.width * 0.5;
    const shoulder = rp.carveShoulder;
    const newChunks = this.terrainStore.snapshotChunksNearPath(pts, halfW, shoulder);
    if (!this._baseTerrainSnapshot) {
      this._baseTerrainSnapshot = newChunks;
      return;
    }
    for (const [key, arr] of newChunks) {
      if (!this._baseTerrainSnapshot.has(key)) {
        this._baseTerrainSnapshot.set(key, arr);
      }
    }
  }
  ...
  _reapplyAllCarves() {
    ...
    const rp = this.toolState.river2;
    const res = this.terrainStore.config.world.dataResolution;

    this.terrainStore.restoreChunkHeightsFromMap(this._baseTerrainSnapshot);

    const dirtyChunks = new Map();
    for (const key of this._baseTerrainSnapshot.keys()) {
      dirtyChunks.set(key, { minIx: 0, maxIx: res, minIz: 0, maxIz: res });
    }
  ...
  /**
   * Call after any change to segment points. Expands the base snapshot to cover
  ...
  _isStylized() {
    return this.toolState.river2.shaderStyle === "Stylized";
  }
  ...
  deleteActiveRiver() {
    ...
    this.dragging = false;
    this._clampActive();
    // Re-apply remaining carves (removes deleted segment's contribution)
    this._updateCarving();
    this._rebuildVisual();
  }
  ...
  finalizeMove() {
    this.dragging = false;
    this._updateCarving();
    // Mesh already reflects terrain from _updateCarving via rebuildAllMeshes
    this._rebuildVisual();
  }
  ...
  importData(data) {
    // Restore any previously carved terrain before replacing segments
    if (this._baseTerrainSnapshot) {
      const dirtyChunks = new Map();
      const res = this.terrainStore.config.world.dataResolution;
      for (const key of this._baseTerrainSnapshot.keys()) {
    ...
    this.dragging = false;
    this._clampActive();
    // Bake carves for loaded segments
    this._updateCarving();
    this._rebuildVisual();
  }

  dispose() {
    // Restore carved terrain on dispose
    if (this._baseTerrainSnapshot) {
      const dirtyChunks = new Map();
      const res = this.terrainStore.config.world.dataResolution;
      for (const key of this._baseTerrainSnapshot.keys()) {
