/**
 * Volumetric cloud V3 — flight-game tuned port of
 * `clouds_terrain_1600-superjet-optimized.html`.
 *
 * Built on top of v1 (`volumetricCloudSystem.js`) — keeps the full-resolution
 * cloud path so mountain silhouettes stay crisp and there is no temporal
 * ghosting at flight speeds. Adds these optimizations on top:
 *
 *  - Cheap occlusion-only cloud material for the god-rays silhouette pass
 *    (~12-step scalar transmittance, no lighting / no fog).
 *  - Empty-space skipping in the main raymarch (sky / terrain rays terminate
 *    sooner; in-cloud sampling is identical → no quality loss).
 *  - Inside-cloud step boost — when the camera is inside the cloud AABB the
 *    step count is multiplied to reduce graininess at close range.
 *  - Sun-offscreen skip — the occlusion pass + god-rays radial blur + their
 *    additive composite are all skipped when the sun NDC falls outside the
 *    expanded screen rect.
 *  - Depth-prepass override material — terrain / foliage / plane shaders are
 *    bypassed during the depth prepass since only depth values matter.
 *  - Baked mask volume via uniform-branch — the analytical mask SDF is
 *    pre-baked into a single 3D texture; one sample replaces two-samples
 *    plus the SDF math. Toggleable; both paths live in the same shader and
 *    the GPU only runs the chosen one (uniform branch).
 *
 * Independent tool state: reads `toolState.volumetricCloudV3` (not the v1 or
 * v2 keys). Layer 13 isolates this cloud + sun mesh from the other systems
 * so all three can coexist (though typically only one is enabled at a time).
 */
 ...
 * @param {object} opts.toolState — reads `toolState.volumetricCloudV3`.
 * @param {() => THREE.Vector3} opts.getSunDir
 * @param {THREE.DirectionalLight} opts.sun
 * @param {THREE.HemisphereLight} opts.hemi
 * @param {() => THREE.Object3D[]} [opts.getOccluderMeshes] — terrain/solids for occlusion pass.
 */
export async function createVolumetricCloudSystemV3({
  renderer,
  scene,
  camera,
  toolState,
  ...
  const p0 = toolState.volumetricCloudV3;
  ...
  function layoutEffectBufferSizes() {
    ...
    const es = THREE.MathUtils.clamp(
      toolState.volumetricCloudV3.effectBufferScale ?? 0.35,
      0.2,
      1,
    );
  ...
  const blueNoiseUrl = new URL("../../../assets/HDR_L_0.png", import.meta.url)
    .href;
  ...
  /* Cheap occlusion-only material step count (≤ MAX_OCC_LOOP). */
  const uOccMaxSteps = uniform(
    Math.max(
      4,
      Math.min(MAX_OCC_LOOP, p0.occlusionRaymarchSteps ?? 12),
    ),
  );
  /* Empty-space skip (main raymarch). */
  const uEmptyStepMul = uniform(p0.emptySpaceStepMul ?? 2.0);
  const uEmptyDensityThreshold = uniform(0.01);
  ...
  /* Baked-mask 3D texture — pre-computes the analytical mask SDF at every
     voxel so the main raymarch reads one texture sample instead of two
     (mask + detailMask) plus the SDF math. */
  ...
  const uBakedMaskMode = uniform(p0.bakedMaskMode ? 1 : 0);

  function makeMaskNoiseGen(seed) {
    const rnd = createSeededRandom(seed);
    ...
    const permTable = new Uint8Array(512);
    for (let i = 0; i < 256; i++)
      permTable[i] = permTable[i + 256] = perm[i];
    return createNoiseGeneratorFromPermutation(permTable);
  }

  function bakeMaskVolume3D() {
    const p = toolState.volumetricCloudV3;
    const size = Math.max(16, Math.min(192, Math.round(p.maskBakeSize ?? 64)));
    const data = new Uint8Array(size * size * size);
    const noiseGenMain = makeMaskNoiseGen(p.maskSeed);
    const noiseGenDet = makeMaskNoiseGen(p.seedDetalhe);
    ...
    for (let z = 0; z < size; z++) {
      for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
          ...
          const sdf =
            p.raio + nP * p.forcaRuido + nD * p.forcaRuidoDetalhe - dist;
          const t = Math.max(
            0,
            Math.min(1, sdf / Math.max(1e-5, p.maskSoftness)),
          );
          const factor = t * t * (3 - 2 * t);
          data[idx++] = Math.floor(factor * 255);
        }
      }
    }
  ...
  /* Debounce mask bakes — slider scrubs would otherwise trigger 60 bakes/s. */
  let _maskBakeTimer = 0;
  function scheduleMaskBake() {
    if (!toolState.volumetricCloudV3.bakedMaskMode) return;
    if (_maskBakeTimer) clearTimeout(_maskBakeTimer);
    _maskBakeTimer = setTimeout(() => {
      _maskBakeTimer = 0;
      bakeMaskVolume3D();
    }, 80);
  }
  ...
  function rebuildMaskTextures() {
    ...
    /* Noise gens changed → baked mask is stale. */
    bakeMaskVolume3D();
  }
  ...
  /* Single density function with uniform-branch mask source. The branch is
     uniform across all pixels so the GPU only runs the chosen path. */
  const getDensity = Fn(([p]) => {
    const maskFactor = float(0.0).toVar();
    If(uBakedMaskMode.greaterThan(0.5), () => {
      maskFactor.assign(maskVolumeTex.sample(p.add(0.5)).r);
    }).Else(() => {
      maskFactor.assign(getMaskFactor(p));
    });
    const texCoord = p
      .add(0.5)
      .mul(uTextureTiling)
  ...
  const cloudColorNode = Fn(() => {
    ...
    const baseStep = rayLength.div(uMaxSteps.max(1));
    ...
    const fade_zone = baseStep.mul(2.0);

    const accumulatedColor = vec3(0.0).toVar();
    const transmittance = vec3(1.0, 1.0, 1.0).toVar();
    const lightDirWorld = normalize(uLightDir);

    /* Travel-based march with empty-space skip: low-density regions advance
       by `uEmptyStepMul × baseStep`; inside the cloud we stay at baseStep.
       Integration math always uses baseStep so in-cloud output is identical
       to the reference — savings come from sky/terrain rays finishing sooner. */
    const travel = bounds.x.add(jitter.mul(baseStep)).toVar();

    Loop(MAX_RM_STEPS, ({ i: ii }) => {
      If(float(ii).greaterThanEqual(uMaxSteps), () => Break());
      If(travel.greaterThanEqual(bounds.y), () => Break());

      const dist_remaining = bounds.y.sub(travel);
      ...
      const isEmpty = density.lessThan(uEmptyDensityThreshold);
      ...
      If(isEmpty.not(), () => {
        ...
        const totalScattering = sunScattering
          ...
          .mul(density)
          .mul(baseStep);
        const fade_alpha = smoothstep(float(0.0), fade_zone, dist_remaining);
        const scaledScatter = totalScattering.mul(fade_alpha);
        const stepTransmittance = exp(
          density.mul(baseStep).mul(uOpacity).mul(EXTINCTION_MULT).negate(),
        );
  ...
  /* Cheap occlusion-only material — silhouette pass. ~12-step scalar
     transmittance, no lighting / no fog. Replaces the full cloudMat raymarch
     during the god-rays occlusion pass. */
  ...
  cloudMesh.layers.set(CLOUD_V3_LAYER);
  cloudMesh.name = "VolumetricCloudVolumeV3";
  scene.add(cloudMesh);
  ...
  sunSphere.layers.set(CLOUD_V3_LAYER);
  sunSphere.name = "VolumetricCloudSunDiscV3";
  scene.add(sunSphere);

  function updateSunDiscWorld() {
    const pv = toolState.volumetricCloudV3;
    sunSphere.position
      .copy(getSunDir())
      .multiplyScalar(pv.sunMeshDistance ?? 8000);
  ...
  function syncUniformsFromToolState() {
    ...
    godRaysDensity.value = p.godRaysDensity ?? 0.98;
    godRaysDecay.value = p.godRaysDecay ?? 0.975;
    godRaysWeight.value = p.godRaysWeight ?? 0.55;
    /* New optimization uniforms — synced every frame so GUI toggles take
       effect without a shader rebuild. */
    uEmptyStepMul.value = p.emptySpaceSkip
      ? (p.emptySpaceStepMul ?? 2.0)
      : 1.0;
    uOccMaxSteps.value = Math.max(
      4,
      Math.min(MAX_OCC_LOOP, p.occlusionRaymarchSteps ?? 12),
    );
  ...
  function syncFrameState(anchorXZ, dtSec) {
    ...
    const insideCloudBox =
      ...
      Math.abs(camera.position.y - _cloudCenterWorld.y) < halfBox &&
      Math.abs(camera.position.z - _cloudCenterWorld.z) < halfBox;
    if (p.insideCloudStepBoost && insideCloudBox) {
      uMaxSteps.value = Math.min(
        MAX_RM_STEPS,
        Math.round(uMaxSteps.value * (p.insideCloudStepMul ?? 1.6)),
      );
    }
    ...
    const dimGod = p.lodFrustumCull && p.frustumDimGodRays && !inFrustum;
    godRaysExposure.value = dimGod ? 0 : (p.godRaysExposureUI ?? 0.58);
  ...
  return {
    ...
    rebuildMaskTextures,
    rebuildMaskVolume: bakeMaskVolume3D,
    scheduleMaskBake,
    dispose,
  };
}
