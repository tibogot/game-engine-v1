...
function createImpostorMaterials(textures, opts) {
  ...
  const { impostorScale, gridVal, atlasSize, cellPad, fullOctahedral = false, instanced = false } = opts;
  ...
  // Per-tree center + scale. Single-mesh (lab/test) = uniforms; instanced forest
  // tier = per-instance attributes so one draw covers every far tree of a slot.
  // (placement of the billboard verts is via the instance matrix either way;
  //  these drive the octahedral view-direction math + per-tree wind phase.)
  const impCenter    = instanced ? attribute("aImpCenter", "vec3") : uCenter;
  const impScale     = instanced ? attribute("aImpScale", "float") : uScale;
  ...
  const positionFn = Fn(() => {
    ...
    const camLocal = mul(sub(cameraPosition, impCenter), div(1, impScale));
    ...
    const phase = add(mul(uTime, uWindFreq), mul(impCenter.x, float(0.37)));
    const phaseZ = add(mul(uTime, mul(uWindFreq, float(0.83))), mul(impCenter.z, float(0.41)));
