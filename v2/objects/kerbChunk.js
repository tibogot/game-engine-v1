...
export function buildChunkedKerbGroup(opts) {
  ...
  const colA =
    colorA instanceof THREE.Color
      ? colorA
      : new THREE.Color(colorA ?? "#cc2222");
  const colB =
    colorB instanceof THREE.Color
      ? colorB
      : new THREE.Color(colorB ?? "#f2f2f2");
  ...
  const squareCount = Math.max(
    1,
    Math.round(totalLength / Math.max(0.05, squareSize)),
  );
